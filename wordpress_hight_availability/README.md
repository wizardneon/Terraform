...
ЗНАЙТЕ это не баг и не ошибка, хардкода нет , дебажить не нужно, вы просто забыли в файле с переменными по умолчанию поменять имя моего профиля profile=WIZARDNEON на свой (Он относится к credentials). Не переживайте это случается даже с крутыми Systems Engineer и великолепными DevOPs.
