...
ЗНАЙТЕ это не баг и не ошибка, хардкода нет , дебажить не нужно, вы просто забыли в файле с переменными по умолчанию, поменять имя моего aws профиля (profile)  profile=WIZARDNEON на свой профиль aws (Он относится к credentials). Не переживайте это случается даже с крутыми Systems Engineer и великолепными DevOPs.

Вопрос зачем мне porfile  в коде не уместен, так как это мое решение и оно работает, пусть это даже "индийский код" возможно данный параметр был или будет необходим в будующих или прошлых версиях или при дебаге/в процессе написания кода.
